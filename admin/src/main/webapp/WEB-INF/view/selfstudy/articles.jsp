...
<%-- 				<li <c:if test="${type == 1}">class="bgColor"</c:if>><a href="/admin/article?type=1">文章管理(网站)</a></li> --%>
				<li><a href="/admin/selfstudy">自学课程列表</a></li>
			</ul>
		</div>
										...
										<div class="am-btn-toolbar">
											<div class="am-btn-group am-btn-group-xs">
												<a	class="am-btn am-btn-default am-btn-xs am-text-secondary"
															onclick="showOrHidden('${obj.id}');"> <span
															class="am-icon-pencil-square-o"></span> <c:if
															test="${obj.is_hidden == 0}">隐藏</c:if>
														<c:if test="${obj.is_hidden == 1}">显示</c:if>
													</a>
...
<script type="text/javascript">
	function submitForm(){
		if($("#search").val() != ''){
			window.location.href="/admin/article?title="+$("#search").val();
		}
	}
	function cover(id){
	...
	function edit(id){
		if(id == 0){
			window.location.href="/admin/selfstudy/edit";
		}else{
			window.location.href="/admin/selfstudy/edit?id="+id;
		}
	}
	function showOrHidden(id) {
		$.getJSON("/admin/selfstudy/hiddenOrShow?courseId=" + id, function(
				result) {
	...
	function deleteMember(id){
		showConfirm("确认删除?",function(){
			$.getJSON("/admin/selfstudy/delete?id=" + id, function(result) {
				if (result.status == 200) {
					location.reload();
				} else {
					showInfo("系统错误....",function(){
