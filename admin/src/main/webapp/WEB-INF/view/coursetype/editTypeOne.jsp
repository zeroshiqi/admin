      ...
      <div class="am-u-sm-12 am-u-md-8 maxwidth">
        <form class="am-form am-form-horizontal" id="myform" method="post" >
         <div class="am-form-group">
          ...
          <div class="am-form-group">
            <label for="q-question" class="am-u-sm-3 am-form-label">课程一级目录</label>
            <div class="am-u-sm-9">
              <input type="text" id="q-title" name="title" placeholder="名称" value="${record.name}">
              <input type="hidden" id="id" name="id" value="${record.id }">
            </div>
          </div>
           <div class="am-form-group">
            <label for="q-question" class="am-u-sm-3 am-form-label">权重值</label>
            <div class="am-u-sm-9">
              <input type="text" id="q-weight" name="weight" placeholder="名称" value="${record.weight}">
            </div>
          </div>
          <div class="am-form-group">
				...
				<select id="showType" name="showType">
					...
					<c:if test="${not empty record.id}">
						<c:if test="${record.show_type == '0'}">
							<option value="0" selected="selected">不显示Banner</option>
							<option value="1">课程</option>
							<option value="2">讲师</option>
							<option value="3">课程包</option>
						</c:if>
						<c:if test="${record.show_type == '1'}">
							<option value="0">不显示Banner</option>
						...
						<c:if test="${record.show_type == '3'}">
							<option value="0">不显示Banner</option>
							<option value="1">课程</option>
							<option value="2">讲师</option>
							<option value="3" selected="selected">课程包</option>
						</c:if>
					</c:if>
				</select>
            </div>
...
function save(){
	...
	if(title == ''){
		showInfo("请输入一级目录标题!",function(){},3000);
		return;
	}
	$("#saveBtn").attr("disabled","disabled");
	$('#myform').ajaxSubmit({
		type:'post',
		url:"/admin/coursetype/save",
	    success:function(data){
	        if(data.status == 200){
	           	window.location.href = "/admin/coursetype";
	        }else{
...
function addType(id,str){
	$.getJSON("/admin/coursetype/findAllTypes?name="+str+"&id="+id,function(result){
		var html = "";
		for(var i=0;i<result.length;i++){
			html += "<option value='"+result[i].id+"'>";
			html += result[i].value;
