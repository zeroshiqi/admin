	...
	<div class="am-cf admin-main">
		...
		<div class="nav-assist">
			<ul>
				...
				<c:if test="${type ==0 }">
					<li class="bgColor"><a href="/admin/catalog?type=0" >课程一级分类管理</a></li>
					<li><a href="/admin/catalog?type=1" >讲师关联课程包管理</a></li>
				</c:if>
				<c:if test="${type ==1 }">
					<li><a href="/admin/catalog?type=0" >课程一级分类管理</a></li>
					<li class="bgColor"><a href="/admin/catalog?type=1" >讲师关联课程包管理</a></li>
				</c:if>
				<c:if test="${type !=1 and type!=0}">
					<li><a href="/admin/catalog?type=0" >课程一级分类管理</a></li>
					<li><a href="/admin/catalog?type=1" >讲师关联课程包管理</a></li>
				</c:if>
						...
						<table class="am-table am-table-striped am-table-hover table-main">
							<thead>
								<tr>
									...
									<th class="table-title" style="width: 15%"><center>课程分类名称</center></th>
									<th class="table-title" style="width: 10%"><center>子标题名称</center></th>
									<th class="table-title" style="width: 5%"><center>分类属性</center></th>
									<th class="table-title" style="width: 10%"><center>权重值</center></th>
									<th class="table-title" style="width: 15%"><center>试卷名称</center></th>
									<th class="table-title" style="width: 10%"><center>展示课程列表方式</center></th>
									<th class="table-title" style="width: 10%"><center>直接关联的课程数量</center></th>
									<th class="table-title" style="width: 10%"><center>包含的二级分类数量</center></th>
<!-- 									<th class="table-title" style="width: 15%"><center>创建时间</center></th> -->
									<%--<th class="table-set" style="width: 15%"><center>操作</center></th>--%>
								</tr>
							</thead>
							<tbody class="stress">
								<tr style="border: 2px solid red;border-bottom-width: 0;">
									<td><center>${record.id}</center></td>
									<td><center>${record.name}</center></td>
									<td><center>${record.subtitle}</center></td>
									<td>
										<center>
											<c:if test="${record.type eq 0}">课程</c:if>
											<c:if test="${record.type eq 1}">讲师</c:if>
										</center>
									</td>
									<td><center>${record.weight}</center></td>
									<td><center>${record.exam_name}</center></td>
									<td>
										<center>
											<c:if test="${record.flag eq 0}">一级目录形式</c:if>
										</center>
										<center>
											<c:if test="${record.flag eq 1}">二级目录形式</c:if>
										</center>
									</td>
									<td><center><a  class="aYes"  href="/admin/catalog/queryCourse?id=${record.id}">${record.courseNum}</a></center></td>
									<td><center><a  class="aYes"  href="/admin/catalogtwo/querySecond?id=${record.id}">${record.sonCount}</a></center></td>
								</tr>
								<tr style="border: 2px solid red;border-top-width: 0;">
									<td colspan=10>
										<center>
											<div class="am-btn-toolbar">
												<div class="am-btn-group am-btn-group-xs">

													<a class="am-btn am-btn-default am-btn-xs am-text-secondary" onclick="locking('${record.id}','${record.name}');">
														<span class="am-icon-pencil-square-o" ></span> 添加考卷
													</a>
													<a class="glyphicon glyphicon-edit" onclick="edit('${record.id}');">
							...
							<tbody>
								<tr>
									...
									<td>
										<center>
											<c:if test="${obj.type == 0}">课程</c:if>
											<c:if test="${obj.type == 1}">讲师</c:if>
										</center>
									</td>
									...
									<td>
										<center>
											<c:if test="${obj.flag eq 0}">一级目录形式</c:if>
										</center>
										<center>
											<c:if test="${obj.flag eq 1}">二级目录形式</c:if>
										</center>
									</td>
									<td><center><a  class="aYes"  href="/admin/catalog/queryCourse?id=${obj.id}">${obj.courseNum}</a></center></td>
									<td><center><a  class="aYes"  href="/admin/catalogtwo/querySecond?id=${obj.id}">${obj.sonCount}</a></center></td>
								</tr>
								<tr>
									<td colspan=10>
										<center>
											<div class="am-btn-toolbar">
												<div class="am-btn-group am-btn-group-xs">
													<a	class="am-btn am-btn-default am-btn-xs am-text-secondary" onclick="isnew('${obj.id}');"> 
														<span class="am-icon-pencil-square-o"></span> 
														<c:if test="${obj.update_mark == 0}">标识更新</c:if>
														<c:if test="${obj.update_mark == 1}"><font style="font-size: 12px;color: red;"><b>取消更新标识</b></font></c:if>
													</a>
...
<script type="text/javascript">
	function edit(id){
		if(id == 0){
			window.location.href="/admin/catalog/edit?type="+$("#type").val();
		}else{
			window.location.href="/admin/catalog/edit?id="+id+"&type="+$("#type").val();
		}
	}
	function deleteA(id){
...
//		   document.getElementById("memo").value = memo;
	...
	//保存试卷到课程
		function saveExam(){
			$('#examForm').ajaxSubmit({
			    ...
			    success:function(data){
			        if(data.status == 200){
			           	window.location.href = "/admin/catalog?type="+$("#type").val()+"&name="+$("#search").val();
			        }else{
		...
		function isnew(id){
			$.getJSON("/admin/catalog/isnew?catalogId=" + id, function(
					result) {
				if (result.status == 200) {
					location.reload();
				} else {
		...
		function submitForm(){
			if($("#search").val() != ''){
				window.location.href="/admin/catalog?name="+$("#search").val()+"&type="+$("#type").val();
			}
		}
</script>
</html>