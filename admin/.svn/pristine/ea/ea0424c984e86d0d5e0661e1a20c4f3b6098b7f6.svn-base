...
public class QuestionFirstTypeController extends BaseController{
	private QuestionService service = Duang.duang(QuestionService.class);
	
	public void index(){
		Integer page = getParaToInt("page", 1);
		long count = service.findQuestionTypeOneCount();
		
		setAttr("list",service.findQuestionTypeOneList(page));
		setAttr("page", new Page(page,count,"/admin/typeone?s=1"));
		render("typeone.jsp");
	}
